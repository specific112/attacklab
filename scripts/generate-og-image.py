...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .badge {
    display: inline-block; background: rgba(106,255,240,.12);
    border: 1px solid rgba(106,255,240,.25); color: #6afff0;
    font-size: 14px; font-weight: 700; padding: 6px 16px;
    border-radius: 20px; letter-spacing: .08em;
    text-transform: uppercase; margin-bottom: 24px;
  }
  h1 { color: #fff; font-size: 56px; font-weight: 900; line-height: 1.1; margin-bottom: 16px; letter-spacing: -.02em; }
  h1 span { color: #6afff0; }
  .sub { color: rgba(255,255,255,.55); font-size: 22px; font-weight: 400; line-height: 1.5; max-width: 700px; margin: 0 auto 28px; }
  .stats { display: flex; gap: 32px; justify-content: center; }
  .stat { text-align: center; }
  .stat-val { color: #6afff0; font-size: 28px; font-weight: 900; }
  .stat-label { color: rgba(255,255,255,.4); font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: .06em; margin-top: 2px; }
  .url { position: absolute; bottom: 28px; right: 32px; color: rgba(255,255,255,.3); font-size: 14px; font-weight: 600; }
</style>
</head>
<body>
  <div class="grid"></div>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .icon { color: #6afff0; font-size: 64px; font-weight: 900; letter-spacing: -.04em; text-shadow: 0 0 40px rgba(106,255,240,.3); }
</style>
</head>
<body><div class="icon">A</div></body>
</html>"""
...
with sync_playwright() as p:
    ...
    page.screenshot(path=os.path.join(out_dir, "og-image.png"), type="png")
    print("  -> public/og-image.png")

    # Apple Touch Icon (180x180)
    print("Generating apple-touch-icon...")
    page2 = browser.new_page(viewport={"width": 180, "height": 180})
    page2.set_content(apple_html, wait_until="networkidle")
    page2.wait_for_timeout(2000)
    page2.screenshot(path=os.path.join(out_dir, "apple-touch-icon.png"), type="png")
